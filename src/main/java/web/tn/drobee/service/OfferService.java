...
public class OfferService implements IOfferService {
	...
	public List<OfferResponse> Listoffers() {
		...
		for (Offer ac : Offers) {
			OfferResponse or = converttoOR(ac);
			lor.add(or);
		}
		return lor;
	}
	...
	public OfferResponse getofferbyid(Long id) {
		...
		OfferResponse or = converttoOR(of);
		return or;
	...
	@Override
	public Boolean Addoffer(Offer a) {
		a.setImage(null);
		long x = offerRepository.count();
		offerRepository.save(a);
		l.info("Adding Offer with ID: " + a.getId());
		if (offerRepository.count() > x)
			return true;
		else
			return false;
	}
	...
	public OfferResponse converttoOR(Offer of) {
